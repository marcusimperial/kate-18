...
const Set = ({ set, i }) => {
    ...
    return (
        <div id={set?.id} className="grid h-[100dvh] pt-7 md:pt-0 grid-rows-[87%_13%] md:grid-rows-[80%_20%] justify-items-center items-center">
            <div data-aos="fade-in" className="grid justify-items-center items-center grid-rows-[auto_minmax(0,_1fr)]">
                ...
                <div className="grid md:grid-cols-3 md:gap-x-6 md:gap-3">
                    { set?.people?.map((person, i) => (<h1 key={`p${i}`} data-aos="fade-in" data-aos-offset="0" data-aos-delay={((i + 1) * 150).toString()} className="text-xl md:text-3xl lg:text-4xl text-center">{person}</h1>)) }
                </div>
            </div>
